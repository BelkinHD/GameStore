...
def misdatos(request):
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            perfil = form_perfil.save(commit=False)
            perfil.usuario = usuario  # Asignar el usuario al perfil
            perfil.save()
            tipo_cuenta = perfil.get_tipo_usuario_display()  # Obtener el nombre del tipo de usuario
            messages.success(request, f'Tu cuenta de {tipo_cuenta} ha sido actualizada con éxito.')
            return redirect('misdatos')  # Redirigir a la misma página para mostrar los cambios
        else:
            messages.error(request, 'Hubo un error al actualizar tus datos. Por favor, revisa los campos.')
    else:
...
def ventas(request):
    ...
    for boleta in boletas:
        boleta_historial = {
            ...
            'subsrito': 'Sí' if boleta.cliente.subscrito else 'No',
...
def obtener_productos(request):
    if request.method == 'GET' and 'categoria_id' in request.GET:
        ...
        # Crear una lista de diccionarios para almacenar los datos de los productos
        data = [
            {
                'id': producto.id,
...
def eliminar_producto_en_bodega(request, bodega_id):
    if request.method == 'GET':
        ...
        # Lógica para eliminar el producto de la bodega
        ...
        return redirect('bodega')  # Ajusta 'bodega' al nombre correcto de tu vista de administración de bodega
...
def enviar_correo_cambio_password(request, user, password):
    try:
        ...
        subject = 'Cambio de contraseña Games Shop'
